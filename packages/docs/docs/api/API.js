...
const Item = (({item}) => {
  ...
  const description = item.length == 3 ? (typeof item[2] == "string" && item[2]) : (item.length == 4 && typeof item[3] == "string" && item[3]);
  const children = item.length > 1 && Array.isArray(item[item.length - 1]) && item[item.length - 1];
...
export const API = ({items}) => {
  return (
    <ul>
       {items && items.map((child, i) => <Item item={child} key={i} />)}
    </ul>
  )
};
