...
<API items={[
  ...
  ["rules", "Object", [
    ...
    ["canDrop", "(targetNode: Node, currentNode: Node) => boolean", "Specifies the conditions for dropping the current Node that is being dragged. Returning `false` will prevent the currently dragged Node from being dropped.Reminder: Nodes cannot be dropped into non-Canvas components even if `canDrop` returns `true`."],
