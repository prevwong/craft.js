...
import { ROOT_NODE, ERRROR_NOT_IN_RESOLVER, ERROR_MOVE_TO_NONCANVAS_PARENT, ERROR_MOVE_OUTGOING_PARENT, ERROR_MOVE_INCOMING_PARENT, ERROR_MOVE_TO_DESCENDANT, ERROR_MOVE_NONCANVAS_CHILD, ERROR_DUPLICATE_NODEID, ERROR_NOPARENT } from "@shared/constants";
  ...
  return {
    ...
    deserialize(json: string): Nodes {
      ...
      return Object.keys(reducedNodes).reduce((accum: Nodes, id) => {
        const { type: Comp, subtype, props, parent, closestParent, nodes, _childCanvas, name } = deserializeNode(reducedNodes[id], options.resolver);
    ...
    getDropPlaceholder: (
      ...
      targetParent = (isTargetCanvas && isWithinBorders || targetNode.id == ROOT_NODE) 
                      ? targetNode : 
