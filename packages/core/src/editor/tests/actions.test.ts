...
describe('actions.addNodeAtIndex', () => {
    ...
    it('should throw if we give an invalid index', () => {
        ...
        expect(() =>
            state((actions) =>
                actions.addNodeAtIndex(leafNode, rootNode.id, -1)
            )
        ).toThrow()
        expect(() =>
...
describe('actions.delete', () => {
    it('should throw if you try to a non existing node', () => {
        expect(() =>
            Actions(emptyState)((actions) => actions.delete(leafNode.id))
        )
    })
    it('should throw if you try to delete the root', () => {
        expect(() =>
            Actions(documentState)((actions) => actions.add(rootNode.id))
        )
    })
